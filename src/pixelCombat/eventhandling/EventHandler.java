...
public class EventHandler {
	private static final float CAMERA_ACCELERATION = 10f;
	...
	public void performBattle() {
		if (!player1.isAlive() || !player2.isAlive()) {
			if (!freezeready) {//
				...
				controller.setGLOBALFREEZE(true);
				freezeready = true;

			}

			if (!controller.isGLOBALFREEZE() && freezeready) {
	...
	public boolean both_idle() {
		return (this.player1.isAlive() && player1.statusLogic.isStanding() && !player1.attackLogic.isAttacking()
				&& !player2.statusLogic.isOnAir() && player1.releasedDusts.isEmpty() && player2.releasedDusts.isEmpty())
				|| (this.player2.isAlive() && player2.statusLogic.isStanding() && !player2.attackLogic.isAttacking()
						&& !player1.statusLogic.isOnAir() && player1.releasedDusts.isEmpty()
						&& player2.releasedDusts.isEmpty());
	}
	...
	public void sound(String url) {
		try {
			...
			AudioInputStream inputStream1 = AudioSystem.getAudioInputStream(this.getClass().getResource(url));
