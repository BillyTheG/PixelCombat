...
public class Pound360 extends Projectile implements ICanExplode{
	...
	public Pound360(Zorro character,
			Vector2d pos, Vector2d target,
			GlobalStates effect, boolean sounding,
			float dir) {
		super(character.projectilManager,character, 30f,	0f, pos, target, effect, 100, 15, -7f, dir, 1);
		own_char = character;
	...
	{
	 this.boxes = manager.boxes.get("360Pound");
	}

	@Override
	public void load_own_images() 
	{
		 this.Images 		= manager.pictures.get("360pound");
		 this.loopBools 	= manager.loopBools.get("360pound");
		 this.loopIndizes 	= manager.loopVariabels.get("360pound");
		 this.times 		= manager.times.get("360pound");
	}
	...
	public void checkDefender(Character defender) {
		if(this.hits(defender))
		{
			...
			if (!defender.statusLogic.isKnockback()) {
				...
				own_char.comboTouch(defender, -5f, 15f);
			}
			this.canHit = false;
		}
	}
