...
import pixelCombat.artworks.NatsuKenKakuLightningArtWork;
...
public class NatsuRainenNoMahoNoPika extends Attack {
	private Natsu natsu;
	...
	private NatsuKenKakuLightningArtWork artWork = new NatsuKenKakuLightningArtWork();
	...
	public void process() {
		...
		case 0:
			if (getUser().isSwitcher()) {
				...
				this.lightning.reset(new Vector2d(this.natsu.pos.x - 0.3F, this.natsu.pos.y - 1.45F),
						this.natsu.statusLogic.isRight());
	...
	private void makeTeleport() {
		...
		this.natsu.pos.y = (this.natsu.enemy.pos.y + 5F);
		if (this.natsu.pos.x <= this.natsu.enemy.pos.x) {
			this.natsu.statusLogic.setMovementStates(MovementStates.RIGHT);
	...
	private void makeLightningAura2() {
		...
		this.lightning_aura_2.reset(
				new Vector2d(this.natsu.pos.x - this.natsu.getDir() * 1.5F, this.natsu.pos.y - 1.35F),
				this.natsu.statusLogic.isRight());
		this.natsu.releasedDusts.add(this.lightning_aura_2);
	}

	private void makeLightningAura1() {
		this.lightning_aura_1.reset(
				new Vector2d(this.natsu.pos.x - this.natsu.getDir() * 1.3F, this.natsu.pos.y - 0.5F),
				this.natsu.statusLogic.isRight());
	...
	private void makeLightningFist() {
		...
		this.natsu.sound("/audio/natsu_rainen_no.wav");
		this.natsu.physics.VX += this.natsu.getDir() * 50.0F;
		this.lightning_fist.reset(new Vector2d(this.natsu.pos.x + this.natsu.getDir() * 4.0F, this.natsu.pos.y + 2.05F),
				this.natsu.statusLogic.isRight());
	...
	public void checkContent() {
		...
		if (!this.natsu.enemy.statusLogic.isKnockback()) {
			...
			if (this.natsu.picManager.getCurrFrameIndex() == 13) {
				this.natsu.enemy.physics.VY = 0.0F;
				this.natsu.comboTouch(this.natsu.enemy, -30.0F, 15.0F);
			}
			if (this.natsu.picManager.getCurrFrameIndex() == 20) {
				this.natsu.enemy.physics.VY = 0.0F;
				this.natsu.enemy.physics.VX = 0.0F;
