  ...
  extends PXObject
{
  ...
  public Character(String name, int lifepoints, float movementspeed, float attackspeed, Vector2d pos, int strength, int MaxLifePoints, int magicpoints, int MaxMagicPoints, float AttackFactor, String HitSound, String AttackSound, String SpecialAttackSound, String DeathSound, ProjectileManager projectileManager)
  {
  ...
  public void disable()
  {
    ...
    if (((Float)this.timeManager.getDisableTime().getY()).floatValue() == ((Float)this.timeManager.getDelayHolder().get(TimeState.DISABLETIME))
      .floatValue())
    {
  ...
  public void blink()
  {
    if (((Float)this.timeManager.getBlinkTime().getY()).floatValue() == ((Float)this.timeManager.getDelayHolder().get(TimeState.BLINKTIME)).floatValue())
    {
      this.statusLogic.setGlobalStatus(GlobalStates.ACTIVE);
      this.statusLogic.setActionStates(ActionStates.STAND);
      this.physics.VX = 0.0F;
  ...
  private void makeDefendBubbles()
  {
    ...
    currentDefendBubble.reset(new Vector2d(getPos().x, getPos().y - 0.5F), 
      this.statusLogic.isRight());
    this.releasedDusts.add(currentDefendBubble);
  }
  
  public boolean canDefend(Projectile projectile)
  {
    if ((!isNotHittable()) && (projectile.statusLogic.isRight()) && 
      (this.statusLogic.isLeft()) && (this.statusLogic.isDefending()) && (!projectileMighty(projectile)))
    {
  ...
  {
    this.switcher = true;
    if ((!this.attackLogic.isSpecialAttacking()) && (!this.attackLogic.isRunAttacking()) && (!this.attackLogic.isAirSpecialAttacking())) {
      this.attackLogic.setAttackStatus(AttackStates.notAttacking);
    }
  ...
  public boolean enoughMagic(String input_seq, boolean airAttack)
  {
    ...
    boolean permittedAttackBeforeRelease = (!this.attackLogic.isBasicAttacking1()) && (!this.attackLogic.isBasicAttacking21()) && (!isJumpAttacking()) ? false : airAttack ? isJumpAttacking() : true;
    if ((energy <= this.actualMagicpoints) && (
      (!this.statusLogic.canNotAttack()) || 
      (permittedAttackBeforeRelease)))
    {
      stopActing();
      return true;
    }
  ...
  {
    if (((this.pos.x <= this.enemy.pos.x) && (getDir() == -1.0F)) || ((this.pos.x >= this.enemy.pos.x) && (getDir() == 1.0F))) {
      return true;
    }
    return false;
  }
  ...
  {
    this.picManager.getClass();return this.picManager.getCurrentAnimation() == 25;
  }
}
